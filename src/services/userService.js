            ...
            _self.$http.post(config.ajax._post.login, {
                ...
                if (!errCode) {
                    switch (data.data.status) {
                        case 0:
                            _self.$router.push({ name: 'register', query: { verify: true } });
                            break;
                        case 1:
                            resolve();
                            ...
                            _self.keepName ? cookieManager.setItem(config.cookie.userKey, username, 30) : cookieManager.removeItem(config.cookie.userKey);
                            redirect ? _self.$router.push(query) : _self.$router.push({ name: 'account' });
                            break;
                    }
                } else {
        ...
        return new Promise((resolve, reject) => {
            ...
            _self.$http.post(config.ajax._post.register, {
                ...
                if (data.data) {
                    ...
                    (errCode) ? errorHandler(errCode) : window.layer.msg('注册失败', { time: 1300 });
                }
            }, (resp) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            _self.$http.post(config.ajax._post.changePwd, {
                ...
                if (data.data) {
                    ...
                    (errCode) ? errorHandler(errCode) : window.layer.msg('密码修改失败', { time: 1300 });
                }
            }, (resp) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            _self.$http.post(config.ajax._post.feedback, {
                ...
                if (data.data) {
                    ...
                    (errCode) ? errorHandler(errCode) : window.layer.msg('反馈失败', { time: 1300 });
                }
            }, (resp) => {
