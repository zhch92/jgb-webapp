<template>
    <div id="register">
    <div class="register form" v-if="pageNo == 0">
        <div class="content">
            ...
            <div class="clearfix">
                <div class="left-box">
                    ...
                    <div class="row">
                        ...
                        <div class="middle">
                            <input type="text" id="company" placeholder="请输入公司名称" maxlength="50">
                        </div>
                        <div class="right"></div>
                    </div>
                    <div class="row">
    ...
    <div class="register agreement" v-if="pageNo == 1">
        <div class="head">
            <p>机构宝服务协议</p>
        </div>
        <div class="content">
...
<script>
    ...
    import { getUrlParam, stringTrim, errorHandler, logger } from '../plugins/utils'
    ...
    export default {
        ...
        beforeMount () {
            (!getUrlParam('verify')) ? this.pageNo = 0 : this.pageNo = 2;
        },
        methods: {
            changeCaptch () {
            ...
            submit () {
                ...
                if (!stringTrim($company.value)) {
                    window.layer.tips('公司名称不能为空', $company);
                } else if (!stringTrim($user.value)) {
                    window.layer.tips('真实姓名不能为空', $user);
                } else if (!stringTrim($mobileNo.value)) {
                    window.layer.tips('手机号码不能为空', $mobileNo);
                ...
                } else if ($rule.className.indexOf('active') == -1) {
                    window.layer.tips('请同意服务协议', $rule, { tips: 1 });
                } else {
                    userService.register($company.value, $user.value, $mobileNo.value, $captcha.value, $smscode.value, $password.value, _self).then(() => {
                        _self.pageNo = 2;
                        setTimeout(function () {
